                ...
                <div class="card-body pt-0">
                    <form class="form-horizontal" action="{{ url('user/'.$data->id)}}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="">Level <span class="tx-danger">*</span></label>
                                    <select name="level" class="form-control @error('level') parsley-error @enderror">
                                        <option value="" >Pilih Level</option>
                                        <option {{$data->level == "Admin" ? 'selected' : ''}} value="Admin" >Admin</option>
                                        <option {{$data->level == "Cashier" ? 'selected' : ''}} value="Cashier">Kasir</option>
                                        <option {{$data->level == "Staff" ? 'selected' : ''}} value="Staff">Gudang</option>
                                    </select>
                                    @error('level')
                                        <ul class="parsley-errors-list filled" id="parsley-id-5">
                                            <li class="parsley-required">{{ $message }}</li>
